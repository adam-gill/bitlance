...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (job.status !== Status.COMPLETED) {
      ...
      return NextResponse.json({ success: false, message: "Only jobs with status COMPLETED can be updated to CLOSED" }, { status: 400 });
    }

    const updatedJob = await prisma.job.update({
      where: { job_id },
      data: { status: Status.CLOSED, freelancer_id:freelancer?.f_id},
    });

    console.log("Job status updated to CLOSED");
    return NextResponse.json({ success: true, updatedJob, message: "Job status updated to CLOSED" }, { status: 200 });
  } catch (error) {
